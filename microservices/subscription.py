...
from models.subscription import Subscriptions, db, DefaultSubscriptions, create_and_populate_default_subscriptions_table
...
def subscribe_user():
    ...
    client_token = requests.get("http://127.0.0.1:5000/get_client_token/123")
    headers = {
    ...
    if not row.id:
        return jsonify({'error': f"There is not defined subscription for localization -> {data.get('localization')} and device_type -> {data.get('device_type')}"})
...
def check_user_subscription_by_email(client_email):
    ...
    if not client:
        return jsonify({'error': 'Client not found', 'has_subscription': False, 'status': 404})
